...
status_writer_t *status_writer_alloc(int accept_fd) {
  ...
  assert(!set_nonblocking(sw->accept_fd));
  ...
  assert(!set_nonblocking(sw->acceptor_stop_pipe[0]));

  sw->state = STATE_CREATED;
  assert(!pthread_mutex_init(&(sw->state_lock), NULL));
  sw->status = -1;
...
void status_writer_run(status_writer_t *sw) {
  ...
  LIST_FOREACH(sink, &(sw->sinks), next_sink) {
    atomic_write(sink->fd, &(status), sizeof(int), &hup);
    close(sink->fd);
  }
