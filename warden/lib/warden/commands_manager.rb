  ...
  module CommandsManager
    ...
    class Flag
      ...
      def initialize(flag_str, delimiter = ".")
        unless flag_str.is_a?(String)
          ...
          msg << ", but received: #{flag_str.class}."
    ...
    def deserialize(command_args, field_delim = ".")
      unless command_args.is_a?(Array)
        msg = "Expected argument to be of type: #{Array}"
        msg << ", but received: #{command_args.class}."
        raise ArgumentError, msg
      end
      ...
      return if Flag.is_help_flag(cmd_name)
    ...
    # Serializes a protocol buffer message into a hash and returns it.
    # Keys of the hash are string representations of the names of fields in the
    # protocol buffer message. Values in the hash are corresponding values of
    # the fields in the protocol buffer message.
