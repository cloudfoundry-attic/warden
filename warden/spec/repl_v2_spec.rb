...
describe Warden::Repl do
  ...
  describe "#process_command" do
    ...
    it "should add command trace to output" do
      ...
      command_info = repl.process_command("simple_test", "--field", "field")
    ...
    it "should convert run command to spawn and stream commands" do
      ...
      repl.should_receive(:deserialize).once.with(["run",
                                                   "--handle", "handle",
                                                   "--script", "script"])
        .and_return([:run, run_request])

      STDOUT.should_receive(:write).once.with("stdout")

      command_info = repl.process_command("run",
                                          "--handle", "handle",
                                          "--script", "script")
    end
    ...
    it "should wrap errors" do
      ...
      repl.should_receive(:deserialize).once.with(["simple_test",
                                                   "--field", "field"])
        .and_raise(RuntimeError.new("error"))

      expect {
        repl.process_command("simple_test", "--field", "field")
      }.to raise_error { |error|
        error.should be_an_instance_of Warden::Repl::ReplError
    ...
    it "should generate prettified global help" do
      ...
      command_info = repl.process_command("--help")
    ...
    it "should generate prettified command help for simple command" do
      ...
      command_info = repl.process_command("simple_test", "--help")
    ...
    it "should generate prettified command help for complex command" do
      ...
      command_info = repl.process_command("mixed_test", "--help")
